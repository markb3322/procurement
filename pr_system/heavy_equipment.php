...
<head>
    ...
    <title>Heavy Equipment Procurement - PDS</title>
...
<div class="main-content">
    <form action="process/save_heavy_equipment.php" method="POST">
        <input type="hidden" name="form_type" value="heavy_equipment">
        <div class="container">
            
            <?php if(isset($_SESSION['msg'])): ?>
                <div class="alert alert-<?php echo $_SESSION['msg_type']; ?> alert-dismissible fade show shadow-sm mb-4" role="alert">
                    <i class="bi bi-info-circle-fill me-2"></i> <?php echo $_SESSION['msg']; unset($_SESSION['msg']); unset($_SESSION['msg_type']); ?>
            ...
            <div class="row mb-4 align-items-center">
                <div class="col-md-6">
                    <h3 class="fw-bold m-0 text-uppercase">Heavy Equipment Form</h3>
                    <p class="text-muted">Procurement tracking for machinery and vehicles.</p>
                </div>
                ...
                <div class="col-lg-8">
                    <div class="card card-premium mb-4">
                        <div class="card-body p-4">
                            <span class="section-label">Equipment Specifications</span>
                            <div class="row g-3">
                                <div class="col-md-6">
                ...
                <div class="col-lg-4">
                    <div class="card card-premium">
                        <div class="card-header-dark d-flex justify-content-between align-items-center">
                            <h6 class="mb-0"><i class="bi bi-truck me-2"></i>Check List</h6>
                            <a href="heavy_equipment_bidocs.php" class="btn btn-sm btn-outline-light" style="font-size: 0.7rem;">HEAVY EQUIPMENT BIDOCS</a>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <?php 
                                        $checklist = ['PR', 'ABC', 'PPMP', 'ACT DES', 'IAR/ARE', 'PDRS', 'APP', 'LETTER REQUEST', 'MANUAL OBR'];
                                        foreach($checklist as $item): ?>
                                        <tr>
                                            <td class="small fw-bold align-middle ps-3"><?php echo $item; ?></td>
                                            <td width="90px">
                                                <select name="status[]" class="form-select form-select-sm status-toggle" onchange="handleChecklist(this)">
                                                    <option value="Not Complete">NC</option>
                                                    <option value="Complete">C</option>
                                                </select>
                                            </td>
                                            <td>
                                                <input type="text" name="doc_remarks[]" class="form-control form-control-sm remark-field" 
                                                       value="Please compile the document." readonly>
                                                <input type="hidden" name="doc_name[]" value="<?php echo $item; ?>">
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                        ...
                        <div class="card-footer bg-white p-3">
                            <button type="submit" class="btn btn-success w-100 fw-bold py-3 shadow-sm text-uppercase">
                                <i class="bi bi-save-fill me-2"></i>Save Equipment Record
                            </button>
                        </div>
                    </div>
                </div>
...
function calculateTotal() {
    ...
    const c = parseFloat(costField.value) || 0;
    totalField.value = (q * c).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
}
...
// --- Checklist Handling ---
function handleChecklist(select) {
    const row = select.closest('tr');
    const remarkField = row.querySelector('.remark-field');
    const docTitle = row.cells[0].innerText;
    ...
    if(select.value === 'Complete') {
        ...
        remarkField.value = "Please compile the " + docTitle + " document.";
        remarkField.style.color = "#dc3545";
        remarkField.style.fontWeight = "normal";
    }
}
document.querySelectorAll('.status-toggle').forEach(s => s.classList.add('status-pending'));
</script>
