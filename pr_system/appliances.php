...
<head>
    ...
    <title>Appliances Procurement - PDS</title>
...
<div class="main-content">
    <form action="process/save_appliances.php" method="POST">
        <input type="hidden" name="form_type" value="appliances">
        <div class="container">

            <?php if(isset($_SESSION['msg'])): ?>
                <div class="alert alert-<?php echo $_SESSION['msg_type']; ?> alert-dismissible fade show shadow-sm mb-4" role="alert">
                    <i class="bi bi-check-circle-fill me-2"></i> <?php echo $_SESSION['msg']; unset($_SESSION['msg']); unset($_SESSION['msg_type']); ?>
            ...
            <div class="row mb-4 align-items-center">
                <div class="col-md-6">
                    <h3 class="fw-bold m-0 text-uppercase">Appliances Form</h3>
                    <p class="text-muted">Procurement and inventory management portal.</p>
                </div>
                ...
                <div class="col-lg-4">
                    <div class="card card-premium">
                        <div class="card-header-dark d-flex justify-content-between align-items-center">
                            <h6 class="mb-0"><i class="bi bi-check2-square me-2"></i>Check List</h6>
                            <a href="appliances_bidocs.php" class="btn btn-sm btn-outline-light" style="font-size: 0.7rem;">APPLIANCES BIDOCS</a>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <?php 
                                        $checklist_items = ['PR', 'ABC', 'PPMP', 'ACT DES', 'IAR/ARE', 'PDRS', 'APP', 'LETTER REQUEST', 'MANUAL OBR'];
                                        foreach($checklist_items as $item): ?>
                                        <tr>
                                            <td class="small fw-bold align-middle ps-3"><?php echo $item; ?></td>
                                            <td width="90px">
                                                <select name="status[]" class="form-select form-select-sm status-input" onchange="toggleChecklist(this)">
                                                    <option value="Not Complete">NC</option>
                                                    <option value="Complete">C</option>
                                                </select>
                                            </td>
                                            <td>
                                                <input type="text" name="doc_remarks[]" class="form-control form-control-sm remark-display" 
                        ...
                        <div class="card-footer bg-white p-3">
                            <button type="submit" class="btn btn-success w-100 fw-bold py-3 shadow-sm text-uppercase">
                                <i class="bi bi-cloud-check-fill me-2"></i>Save Appliance Record
                            </button>
                        </div>
                    </div>
                </div>
...
function updatePrice() {
    ...
    totalEl.value = (qty * cost).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
}
...
// --- Checklist Logic ---
function toggleChecklist(select) {
    const row = select.closest('tr');
    const remarkField = row.querySelector('.remark-display');
...
document.querySelectorAll('.status-input').forEach(s => s.classList.add('status-pending'));
</script>
